...
export const createUser = (req, res) => {
  ...
  uDB.push(`/used/${email}`, id);// This is for quicker lookup in demo
...
export const updateHistory = (req, res) => {
  ...
  const { info } = req.body;
  const { cart, history } = req.userObj;
  ...
  history.push(entry);
  uDB.push(`/users/${userId}/history`, history);
...
export const updateCart = (req, res) => {
  ...
  const prevItem = req.userObj.cart[item.id];
